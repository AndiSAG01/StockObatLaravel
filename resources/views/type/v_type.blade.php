  ...
  <x-alert></x-alert>
    <div class="card mb-4">
      <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        ...
        <a href="{{ route('type.create') }}" class="btn btn-primary btn-sm">Tambah Data</a>
      </div>
        ...
        <table id="example" class="table align-items-center table-flush table-hover" id="dataTableHover">
          ...
          <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->name }}</td>
                <td>
                    <a href="{{ route('type.edit', $item->id ) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>
                    <form id="deleteForm{{ $item->id }}" class="d-inline" action="{{ route('type.delete',$item->id) }}" method="post">
                        @csrf
                        @method('delete')
